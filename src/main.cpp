...
// RENDERING - walls
//
// Walls are shaded two ways: by distance (existing fog dither),
// and now also by which face was hit (N/S vs E/W), using a
// mortar-line pattern on one orientation instead of a solid
// fill. This is the classic raycaster "side shading" trick,
// done with a pattern instead of brightness since the display
// is 1-bit. It's not more expensive than before - the mortar
// loop skips every 4th row, so it touches FEWER pixels than a
...
void castAndDrawWalls() {
  for (uint8_t col = 0; col < 128; col++) {
    ...
    int16_t dist = 0;
    bool hit = false;
    bool sideVertical = false; // true = hit a wall face by crossing a column (X) boundary
    int16_t prevCx = rx >> 8;
    ...
    while (dist < MAX_RAY_DIST) {
      ...
      dist += RAY_STEP;
      int16_t curCx = rx >> 8;
      if (isWall(rx, ry)) { sideVertical = (curCx != prevCx); hit = true; break; }
      prevCx = curCx;
    }
...
// shape, instead of one big rectangle behind the whole gun
void drawGunPart(int16_t x, int16_t y, int16_t w, int16_t h) {
  u8g2.setDrawColor(0);
